...
$sql = "select * from tbl_product where status=0 order by sort,date_added desc limit ".$row*$col*$p.",".$row*$col;
$result = @mysql_query($sql,$conn);

$total = countRecord("tbl_product","status=0");
if($total==0){
?>
...
<?php for($j=0; $j<$col&&$products=mysql_fetch_assoc($result); $j++){
		...
		<table width="90%" border="0" cellspacing="0" cellpadding="0" style="float:left">
			...
			<tr>
			  <td class="style13" width="120"><a href="./?frame=product_detail&id=<?php echo $pro['id']?>" title="<?php echo $pro['name']?>">  
			  	<?php 
				...
				<td>
					<table>
					<tr><td><a href="./?frame=product_detail&id=<?php echo $pro['id']?>" class="link4"><?php echo $pro['name']?></a></td></tr>
