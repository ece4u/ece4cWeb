...
//	$catInfo       = getRecord($tableCategoryConfig, 'id='.$parent);
	...
	if ($errMsg==''){
		if (!empty($_POST['id'])){
			$oldid = $_POST['id'];
			$sql = "update ".$tableConfig." set name='".$name."',short_description='".$short_description."',full_description='".$full_description."',lang='".$lang."' where id='".$oldid."'";

		}else{

			$sql = "insert into ".$tableConfig." (name, short_description, full_description, lang) values ('".$name."','".$short_description."','".$full_description."','".$lang."')";
